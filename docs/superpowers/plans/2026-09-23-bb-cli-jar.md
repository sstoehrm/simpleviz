...
- Release jar file name exactly `simpleviz.jar` (bbin names the command after the URL's file name before the first dot), with `Main-Class: simpleviz.main`. bbin's shim adds the jar to the classpath after bb has loaded any `bb.edn` in the caller's folder, so `simpleviz.main` re-runs `bb -cp <jar> -m cli …` when one was loaded (its `:paths` would shadow the jar) and calls `cli/-main` otherwise.
