...
| path above the served folder, or a fork | `node api: pair "../x.edn#a" leaves the served folder` / `… names a fork` |
| file missing | `node api: pair "deploy.edn#api-svc": deploy.edn not found` |
| file doesn't parse | `node api: pair "deploy.edn#api-svc": deploy.edn does not parse` |
| id missing | `node api: pair "deploy.edn#api-svc": no node or box api-svc in deploy.edn` |

`simpleviz check` checks the outgoing pairs of the file it is given,
resolving paths against that file (no served folder, so no boundary
warning there). It doesn't scan for incoming pairs.
...
- In a suffix comparison, following a pair opens the target file's own
  ...
  file's comparison.
- A graph served from a PNG shows its pairs but, as with refs, can't
  follow them. PNG files aren't scanned for incoming pairs; a pair may
...
- `parse`: a `:pair` value → `[{:file :id :raw}]` and warnings for
  malformed values.
- `target-path`: a pair path joined to the declaring file's folder
  (root-relative, `.`/`..` collapsed as `editor/resolve-ref` does for
  refs), then checked with `serve/resolve-path`, the canonical
  inside-the-served-folder check `?file=` already uses, and against
  `serve/fork-name` for forks.
- `index`: the reverse index of a folder. Every `.edn` under the served
  root's folder, recursively, skipping dot-folders and, with a
  comparison suffix active, fork files (`*-<suffix>.edn`). Each file is
...
Payload, on each node and box of `/api/graph`:
    ...
    :pairs [{:dir "out" :file "views/deploy.edn" :id "api-svc" :kind "node"}
            ...
            {:dir "out" :file "gone.edn" :id "x" :problem "gone.edn not found"}]

For `"out"` pairs, `:kind` is the target's; for `"in"` pairs, it's the
declaring element's. `"in"` pairs never carry a `:problem`.

`check/check` runs `attach` with an empty index: it checks outgoing
pairs only, reading their target files relative to the checked file.
