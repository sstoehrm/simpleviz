...
**Interfaces:**
- Produces: `serve/version` (public, `[] -> string`), `serve/start!` (`[{:keys [file suffix port debug]}] -> {:served string :log-path string-or-nil}`, throws `ex-info` with `{:startup-check true}` for a side that doesn't resolve or read, lets `java.net.BindException` through), `proc-util/repo-root`, `proc-util/free-port`, `proc-util/start`, `proc-util/await-line`.
